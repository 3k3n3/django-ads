...
class AdvertiserAdmin(admin.ModelAdmin):
    ...
    def get_changeform_initial_data(self, request):
        ...
        return get_data or {"created_by": request.user.pk}
...
class ClickAdmin(admin.ModelAdmin):
    ...
    def download_clicks(self, request, queryset):
        ...
        for click in queryset:
            writer.writerow(
                (
                    (click.ad.title).encode("utf-8"),
                    click.ad.url,
                    click.source_ip,
                    click.click_date.isoformat(),
                    click.ad.advertiser.pk,
                    (click.ad.advertiser.company_name).encode("utf-8"),
                    (click.ad.zone).encode("utf-8"),
                )
            )
        return response
...
class ImpressionAdmin(admin.ModelAdmin):
    ...
    def download_impressions(self, request, queryset):
        ...
        for impression in queryset:
            writer.writerow(
                (
                    (impression.ad.title).encode("utf-8"),
                    impression.ad.url,
                    impression.source_ip,
                    impression.impression_date.isoformat(),
                    impression.ad.advertiser.pk,
                    (impression.ad.advertiser.company_name).encode("utf-8"),
                    (impression.ad.zone).encode("utf-8"),
                )
            )
        return response
